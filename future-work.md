...
- **Next action:** EAS build **#37** (production, auto-submit). Build #36 was attached+saved to version 1.0 (NOT submitted); swap **36 → 37** after it processes. ALSO (first submission): both subscriptions (`finify_premium_monthly`/`yearly`) are **Missing Metadata** — they have EN+TR localization + a review screenshot already, but need pricing completed → then bundle them with v1 on the version page → **Add for Review** → submit. Apple requires the first subscription to be submitted with the app version.
